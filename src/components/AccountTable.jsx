          ...
          <tr key={idx} className="hover:bg-slate-700/20 transition-colors">
            <td className="p-2">
              <div className="flex justify-center gap-1 bg-black/40 p-1 rounded-sm w-fit mx-auto">
                <div className={`w-2.5 h-2.5 rounded-full ${row.online ? 'bg-slate-700' : 'bg-red-600 shadow-[0_0_5px_rgba(220,38,38,0.8)] animate-pulse'}`} />
                <div className={`w-2.5 h-2.5 rounded-full ${row.online ? 'bg-green-500 shadow-[0_0_5px_rgba(34,197,94,0.8)]' : 'bg-slate-700'}`} />
              </div>
            </td>
            <td className="p-2 font-bold text-blue-400">{row.name}</td>
            <td className={`p-2 font-medium ${row.online ? 'text-green-500' : 'text-red-500'}`}>{row.status}</td>
            <td className="p-2 text-slate-400">{row.lastFetched || 'N/A'}</td>
            <td className="p-2 text-slate-400">{row.lastSent || 'N/A'}</td>
            <td className="p-2 text-right">
              <button className="text-blue-500 hover:underline text-[10px] font-bold">VIEW</button>
            </td>
          </tr>
        ))}
      </tbody>
