...
export default function Account() {
  ...
  const handleDelete = (id) => {
    ...
    if (gatewayApi.deleteAccount(id)) {
      ...
      setSelectedId(null);
      showErrorToast(t("accounts.toast.deleteFailed") + (response.message || "Unknown error"), toast);
    }
  };
    ...
    <DashboardLayout>
      <div className="flex h-full bg-slate-100 text-slate-900 p-4 gap-4 relative m-h-[70vh]">
            ...
            <table className="w-full text-left text-sm border-collapse">
              <thead>
                <tr className="bg-gray-50 text-gray-500 uppercase tracking-wider border-b border-gray-200">
                  <th className="px-4 py-3 font-semibold text-xs">{t("accounts.table.accountName")}</th>
                  <th className="px-4 py-3 font-semibold text-xs">{t("accounts.table.protocol")}</th>
                  <th className="px-4 py-3 font-semibold text-xs">{t("accounts.table.status")}</th>
                  <th className="px-4 py-3 font-semibold text-xs">{t("accounts.table.bindStatus")}</th>
                  <th className="px-4 py-3 font-semibold text-xs">{t("accounts.table.host")}</th>
                  <th className="px-4 py-3 font-semibold text-xs">{t("accounts.table.port")}</th>
                </tr>
              </thead>
        ...
        <div className="w-48 flex flex-col gap-2">
          <h3 className="text-[10px] font-bold text-slate-500 uppercase px-2 mb-1">{t("accounts.commands.title")}</h3>
          
          <div className="flex flex-col gap-1 bg-slate-900 p-2 rounded-lg border border-slate-800">
            <CommandBtn icon={<Plus size={14} />} label={t("accounts.commands.add")} color="text-green-400" onClick={handleAdd} disabled={false} />
            <CommandBtn icon={<Edit3 size={14} />} label={t("accounts.commands.edit")} onClick={handleEdit} disabled={!selectedId} />
            <CommandBtn 
              icon={<Trash2 size={14} />} 
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="bg-slate-900 border border-slate-700 w-full max-w-md rounded-xl shadow-2xl overflow-hidden">
              ...
              <form onSubmit={handleSaveAccount} className="p-4 flex flex-col gap-4 text-sm">
                <div>
                  <label className="text-slate-400 block mb-1">{t("accounts.modal.accountName")}</label>
                  <input 
                    required
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-slate-400 block mb-1">{t("accounts.modal.protocol")}</label>
                    <select
                      className="w-full bg-slate-800 border border-slate-700 rounded p-2 text-black focus:border-blue-500"
                  ...
                  <div>
                    <label className="text-slate-400 block mb-1">{t("accounts.modal.port")}</label>
                    <input
                      type="number"
                      className="w-full bg-slate-800 border border-slate-700 rounded p-2 text-black focus:border-blue-500"
                      value={formData.port}
                ...
                <div>
                  <label className="text-slate-400 block mb-1">{t("accounts.modal.host")}</label>
                  <input 
                    required
                    className="w-full bg-slate-800 border border-slate-700 rounded p-2 text-black focus:border-blue-500"
                    value={formData.host}
                ...
                <div>
                  <label className="text-slate-400 block mb-1">{t("accounts.modal.username")}</label>
                  <input
                    type="text"
                    className="w-full bg-slate-800 border border-slate-700 rounded p-2 text-black focus:border-blue-500"
                ...
                <div>
                  <label className="text-slate-400 block mb-1">{t("accounts.modal.password")}</label>
                  <input
                    type="password"
                    className="w-full bg-slate-800 border border-slate-700 rounded p-2 text-black focus:border-blue-500"
                ...
                <div>
                  <label className="text-slate-400 block mb-1">{t("accounts.modal.vpn")}</label>
                  <input
                    type="text"
                    className="w-full bg-slate-800 border border-slate-700 rounded p-2 text-black focus:border-blue-500"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-slate-400 block mb-1">{t("accounts.modal.status")}</label>
                    <select
                      className="w-full bg-slate-800 border border-slate-700 rounded p-2 text-black focus:border-blue-500"
                  ...
                  <div>
                    <label className="text-slate-400 block mb-1">{t("accounts.modal.bindStatus")}</label>
                    <select
                      className="w-full bg-slate-800 border border-slate-700 rounded p-2 text-black focus:border-blue-500"
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <label htmlFor="tlsEnabled" className="text-slate-400">{t("accounts.modal.tlsEnabled")}</label>
                </div>
    ...
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="rounded-xl bg-slate-900/95 p-6 text-center text-white border border-slate-700 shadow-2xl">
        <div role="status" className="flex flex-col items-center gap-4">
          ...
          <span className="text-sm font-medium">{t("accounts.modal.saving")}</span>
        </div>
      </div>
    </div>
  );
