...
import { XCircle, Copy, RefreshCcw, Calendar, FolderOpen, Search, Filter, Info, ChevronRight } from 'lucide-react';
...
const FullLogView = () => {
  ...
  const handleCopy = () => {
    if (selectedLog) {
      navigator.clipboard.writeText(`${selectedLog.timestamp} [${selectedLog.level}] ${selectedLog.message}`);
    ...
    <DashboardLayout>
        ...
        <div className="flex-1 overflow-auto custom-scrollbar relative bg-slate-100">
            ...
            <tbody>
                ...
                >
                    ...
                    <td className={`px-4 py-1 ${selectedLog?.id === log.id ? 'text-white' : 'text-slate-500'}`}>
                    {log.timestamp}
                    </td>
                    ...
                    <td className={`px-4 py-1 ${selectedLog?.id === log.id ? 'text-blue-100' : 'text-purple-400/80'}`}>
                    {log.thread}
                    </td>
                    <td className={`px-4 py-1 whitespace-pre-wrap ${
                    selectedLog?.id === log.id ? 'text-white' : 'text-slate-300'
                    }`}>
                    {log.message}
                    </td>
                </tr>
        ...
        {selectedLog && (
            ...
            <div className="flex-1">
                <h4 className="text-[10px] font-bold text-blue-400 uppercase mb-1">Selected Message Detail</h4>
            ...
            <div className="w-48 flex flex-col justify-end gap-2 text-[10px]">
                ...
                <span className="text-green-500 font-bold tracking-tighter">READING</span>
                </div>
                <button 
                onClick={handleCopy}
...
const ToolbarBtn = ({ icon, label, onClick, disabled = false, active = false, variant = "default" }) => (
  <button 
    disabled={disabled}
    onClick={onClick}
    className={`
